...
export default function PageEditorPage() {
  const [, routeParams] = useRoute('/admin/pages/:id/edit');
  const [, newParams] = useRoute('/admin/pages/new');
  ...
  const saveMutation = useMutation({
    mutationFn: async (payload: PageData & { publish?: boolean }) => {
      ...
      const body = {
        ...payload,
        isPublished,
        content: payload.content || ' ', // Ensure non-empty for validation
      };
    ...
    onSuccess: (data, variables) => {
      ...
      if (isNew && savedPage.id) {
        setLocation(`/admin/pages/${savedPage.id}/edit`);
      }
    },
    onError: (error: Error) => {
      toast({
