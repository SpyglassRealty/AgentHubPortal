...
export default function CoreImportPage() {
  ...
  const handleImport = async () => {
    ...
    try {
      const res = await fetch("/api/admin/core/import-sheet", {
        method: "POST",
      ...
      if (data.successCount > 0) {
        toast({
          ...
          description: `${data.successCount} of ${data.total} core pages imported as drafts`,
        });
      }
    } catch (err: any) {
      toast({
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold flex items-center gap-2">
            <Layout className="h-6 w-6 text-[#EF4923]" />
            Import Core Pages from Google Sheet
          </h1>
          <p className="text-sm text-muted-foreground">
            Paste a Google Sheets URL to import core pages into the page builder as drafts
          </p>
        </div>
      </div>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="text-xs text-muted-foreground space-y-1">
            <p><strong>Expected columns:</strong> Page Title, URL Slug, Google Doc (HYPERLINK), Hero Image (HYPERLINK), OG Image (HYPERLINK), Page Photos (HYPERLINK), Date Crawled</p>
            <p><strong>Requirements:</strong> The sheet and all linked Google Docs must be shared publicly (Anyone with the link → Viewer)</p>
            <p><strong>Note:</strong> Pages will be created with <code>pageType: core</code> and landing-page-style layouts (hero + content sections + CTA)</p>
          </div>
        </CardContent>
      </Card>
        ...
        <Card>
          <CardContent className="py-12 flex flex-col items-center gap-4">
            ...
            <div className="text-center">
              <p className="font-medium">Importing core pages...</p>
              <p className="text-sm text-muted-foreground">Fetching sheet data, downloading Google Docs, building landing page layouts</p>
            </div>
          </CardContent>
        </Card>
      )}
                  ...
                  <TableBody>
                    {results.map((result, idx) => (
                      <TableRow key={idx}>
                        ...
                        <TableCell>
                          {result.success ? (
                            <Link href={`/admin/pages/core/${result.pageId}/edit`}>
