...
export default function CMAPresentationBuilder() {
  ...
  // In AgentHubPortal, CMAs are standalone (no transaction linkage).
  // Subject property data comes from cma.subjectProperty directly.
  const linkedTransaction: any = null;
  ...
  const exportPdfMutation = useMutation({
    mutationFn: async () => {
      ...
      a.href = url;
      a.download = `CMA-${(cma?.subjectProperty as any)?.mlsNumber || id}.pdf`;
  ...
  const subjectProperty = subjectFromTransaction || properties.find(p => 
    p.mlsNumber === (cma?.subjectProperty as any)?.mlsNumber
  );
  ...
  const subjectMlsNumber = (subjectProperty as any)?.mlsNumber || 
                           ...
                           (cma?.subjectProperty as any)?.mlsNumber;
  ...
  const comparables = subjectFromTransaction 
    ...
    : propertiesWithLocations.filter(p => p.mlsNumber !== (cma?.subjectProperty as any)?.mlsNumber);
        ...
        <div className="flex items-center justify-between gap-4 max-w-7xl mx-auto">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">{cma.name || (cma.subjectProperty as any)?.mlsNumber || ''}</p>
            </div>
          </div>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="hidden lg:flex lg:w-2/5 p-4 bg-muted/30 flex-col">
          <LivePreviewPanel
            ...
            transactionId={cma?.id || undefined}
          />
        </div>
      </div>
