...
router.post("/admin/blog/import-sheet", async (req, res) => {
  try {
    ...
    for (let r = range.s.r + 1; r <= range.e.r; r++) {
      ...
      const blogPhotosCell = getCell(5);
      const datePublishedCell = getCell(6);
      ...
      // Extract Blog Photos folder URL
      let blogPhotosUrl = "";
      if (blogPhotosCell?.f) {
        blogPhotosUrl = extractUrlFromFormula(blogPhotosCell.f);
      } else if (blogPhotosCell?.l?.Target) {
        blogPhotosUrl = blogPhotosCell.l.Target;
      } else if (blogPhotosCell?.v && typeof blogPhotosCell.v === "string" && blogPhotosCell.v.startsWith("http")) {
        blogPhotosUrl = blogPhotosCell.v;
      }

      // Extract Date Published
      const datePublished = datePublishedCell?.v?.toString() || "";

      if (title) {
        dataRows.push({ title, slug: rawSlug, googleDocUrl, heroImageUrl, ogImageUrl, blogPhotosUrl, datePublished });
      }
    }
    ...
    for (let i = 0; i < dataRows.length; i++) {
      const { title, slug: rawSlug, googleDocUrl, heroImageUrl: rawHeroUrl, ogImageUrl: rawOgUrl, blogPhotosUrl, datePublished: datePublishedStr } = dataRows[i];
      ...
      if (datePublishedStr) {
        const cleaned = datePublishedStr.replace(/(\d+)(st|nd|rd|th)/g, "$1");
      ...
      try {
        const [newPage] = await db
          ...
          .values({
            ...
            isPublished: false,
            publishDate: parsedPublishDate || (publishDateRaw ? new Date(publishDateRaw) : new Date()),
